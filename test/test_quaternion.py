...
from emath import DQuaternionArray
...
class QuaternionTest:
    ...
    def __init_subclass__(this_cls, *, cls, struct_format):
        this_cls.cls = cls
    ...
    def test_hash(self) -> None:
        for i in range(-100, 100):
            ...
            assert hash(self.cls(*(i for _ in range(4)))) == hash(self.cls(*(i for _ in range(4))))

        assert hash(self.cls(0)) != hash(self.cls(-1))
        assert hash(self.cls(0)) != hash(self.cls(1))
        assert hash(self.cls(1)) != hash(self.cls(-1))
        assert hash(self.cls(0)) != hash(self.cls(*range(4)))
    ...
    def test_repr(self) -> None:
        quat = self.cls(*range(4))
        assert repr(quat) == (
            f"{quat.__class__.__name__}(" + ", ".join(repr(quat[i]) for i in range(4)) + f")"
        )
    ...
    def test_equal(self) -> None:
        for i in range(-100, 100):
            ...
            assert self.cls(*(i for _ in range(4))) == self.cls(*(i for _ in range(4)))
    ...
    def test_not_equal(self) -> None:
        for i in range(-100, 100):
            ...
            assert not (self.cls(*(i for _ in range(4))) != self.cls(*(i for _ in range(4))))
    ...
    def test_multiply(self) -> None:
        ...
        assert self.cls(1) * self.cls(1).rotate(radians(90), vector3_cls(1, 0, 0)) == self.cls(
            1
        ).rotate(radians(90), vector3_cls(1, 0, 0))
        assert self.cls(1).rotate(radians(90), vector3_cls(1, 0, 0)) * self.cls(1) == self.cls(
            1
        ).rotate(radians(90), vector3_cls(1, 0, 0))
        assert self.cls(1).rotate(radians(90), vector3_cls(1, 0, 0)) * self.cls(1).rotate(
            radians(90), vector3_cls(0, 1, 0)
        ) == self.cls(1).rotate(radians(90), vector3_cls(1, 0, 0)).rotate(
            radians(90), vector3_cls(0, 1, 0)
        )
    ...
    def test_matrix_multiply(self) -> None:
        ...
        assert self.cls(1) @ self.cls(1) == sum(self.cls(1)[i] * self.cls(1)[i] for i in range(4))
        assert self.cls(*range(4)) @ self.cls(*range(4)) == sum(
            self.cls(*range(4))[i] * self.cls(*range(4))[i] for i in range(4)
        )
    ...
    def test_buffer(self) -> None:
        assert bytes(self.cls(*range(4))) == struct.pack(self.struct_format * 4, *range(4))
    ...
    def test_magnitude(self) -> None:
        assert isclose(self.cls(-1, -1, -1, -1).magnitude, sqrt(sum(1**2 for _ in range(4))))
        assert isclose(self.cls(1, 1, 1, 1).magnitude, sqrt(sum(1**2 for _ in range(4))))
        assert isclose(self.cls(-2, -2, -2, -2).magnitude, sqrt(sum(2**2 for _ in range(4))))
        assert isclose(self.cls(2, 2, 2, 2).magnitude, sqrt(sum(2**2 for _ in range(4))))
        assert isclose(self.cls(*range(4)).magnitude, sqrt(sum(i**2 for i in range(4))))
    ...
    def test_array_size(self) -> None:
        ...
        assert self.array_cls(self.cls()).size == (struct.calcsize("=" + self.struct_format) * 4)
        assert self.array_cls(self.cls(), self.cls()).size == (
            struct.calcsize("=" + self.struct_format) * 4 * 2
        )

    def test_from_buffer(self) -> None:
        for v in (self.cls(), self.cls(1), self.cls(*range(4))):
    ...
    def test_from_array_buffer(self) -> None:
        for a in (
            ...
            self.array_cls(self.cls(1), self.cls(*range(4))),
        ):
...
class TestDQuaternion2(QuaternionTest, cls=DQuaternion, struct_format="d"):
    pass
